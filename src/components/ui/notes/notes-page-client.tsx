...
import {
  ...
  deleteNote,
  getUsers,
} from "@/app/(dashboard)/[orgSlug]/notes/actions";
...
export default function NotesPageClient({
  ...
  // sees the whole org's notes — only their own.
  const mayModify = (note: Note) => canModifyRecord(currentUser, note.user_id);
  ...
  const handleModalSave = async (formData: FormData) => {
    ...
    const isEditingFlag =
      formData.get("_editing") || formData.get("id") || editing;
  ...
  const handleUpdate = async (formData: FormData): Promise<ActionResult> => {
    ...
    if (existing) {
      ...
      if (formData.has("is_pinned"))
        updatedNote.is_pinned = formData.get("is_pinned") === "1" ? 1 : 0;
      if (formData.has("is_archived"))
        updatedNote.is_archived = formData.get("is_archived") === "1" ? 1 : 0;
      if (formData.has("is_favorite"))
        updatedNote.is_favorite = formData.get("is_favorite") === "1" ? 1 : 0;
  ...
  const handleDelete = async (formData: FormData): Promise<ActionResult> => {
    ...
    // its request does.
    await mutate((list) => (list ?? []).filter((n) => n.id !== id), {
      revalidate: false,
    });

    toast.undoable(
      `Note "${existing.title || "Untitled"}" deleted`,
      () => {
        // Undo action
        ...
        void mutate(
          (list) => [existing, ...(list ?? []).filter((n) => n.id !== id)],
          { revalidate: false },
        );
  ...
  const filteredNotes = useMemo(() => {
    let notes = Array.isArray(optimisticNotes)
      ? optimisticNotes.filter(Boolean)
      : [];
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6 md:mb-8">
          <div className="relative flex-1 md:flex-none md:w-64 hidden md:block">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-white/[0.03] border border-card-border rounded-xl focus:outline-none focus:border-card-border text-foreground placeholder:text-text-muted/50 transition-all"
            />
          </div>

          <div className="flex items-center gap-3 md:gap-4 ml-auto">
            <div className="flex bg-foreground/[0.03] p-1 rounded-xl border border-card-border h-9 lg:h-11">
              <button
            ...
            <button
              ...
              className="flex items-center gap-2 p-1.5 h-9 lg:h-11 rounded-xl bg-foreground/[0.03] hover:bg-foreground/[0.06] border border-card-border hover:border-card-border text-xs md:text-sm text-text-muted hover:text-foreground transition-all duration-200 group"
            >
          ...
          <>
            ...
            {filteredNotes.length === 0 && (
              <EmptyState
                ...
                description="Create a note to get started or adjust your filters."
              />
            )}
          </>
        ) : (
            ...
            <table className="w-full border-collapse">
              ...
              <thead>
                <tr className="border-b border-card-border bg-foreground/[0.03]">
                  <th
                    scope="col"
                    className="px-6 py-4 text-left text-[11px] font-bold text-text-muted uppercase tracking-wider"
                  >
                    Title
                  </th>
                  <th
                  ...
                  <th
                    scope="col"
                    className="px-6 py-4 text-right text-[11px] font-bold text-text-muted uppercase tracking-wider"
                  >
                    Actions
                  </th>
                </tr>
